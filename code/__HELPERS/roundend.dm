...
/datum/controller/subsystem/ticker/proc/survivor_report(popcount)
	...
	parts += "[FOURSPACES]Целостность станции: <B>[GLOB.station_was_nuked ? span_redtext("Destroyed") : "[popcount["station_integrity"]]%"]</B>"
	...
	if(total_players)
		...
		if(SSblackbox.first_death)
			...
			//ignore this comment, it fixes the broken sytax parsing caused by the " above
				...
				parts += "[FOURSPACES]<i>В эту смену никто не умер!</i>"
...
/datum/controller/subsystem/ticker/proc/law_report()
	...
	for (var/i in GLOB.ai_list)
		...
		if(aiMind)
			parts += "<b>[aiPlayer.name]</b> (Играет: <b>[aiMind.key]</b>) законы [aiPlayer.stat != DEAD ? "в конце раунда" : "когда это были [span_redtext("deactivated")]"]:"
		...
		if (aiPlayer.connected_robots.len)
			...
			parts += "<br> Миньонами <b>[aiPlayer.real_name]</b> были:"
			for(var/mob/living/silicon/robot/robo in aiPlayer.connected_robots)
				borg_num--
				if(robo.mind)
					parts += "<b>[robo.name]</b> (Играет: <b>[robo.mind.key]</b>)[robo.stat == DEAD ? " [span_redtext("(Deactivated)")]" : ""][borg_num ?", ":""]"
		if(!borg_spacer)
			borg_spacer = TRUE

	for (var/mob/living/silicon/robot/robo in GLOB.silicon_mobs)
		if (!robo.connected_ai && robo.mind)
			parts += "[borg_spacer?"<br>":""]<b>[robo.name]</b> (Играет: <b>[robo.mind.key]</b>) [(robo.stat != DEAD)? "[span_greentext("survived")] как борг без ИИ!" : "было ли [span_redtext("unable to survive")] тяжело быть киборгом без ИИ."] Его законами были:"
...
/datum/controller/subsystem/ticker/proc/market_report()
	...
	for(var/venue_path in SSrestaurant.all_venues)
		...
		parts += "[venue] обслужило [venue.customers_served] клиентов и заработало [venue.total_income] кредитов.<br>"
	parts += "В общей сложности они заработали [tourist_income] кредитов[tourist_income ? "!" : "..."]<br>"
	log_econ("Доход от обслуживания в обе стороны: [tourist_income] кредиты.")
	switch(tourist_income)
		if(0)
			parts += "[span_redtext("Сервис не заработало никаких кредитов...")]<br>"
		if(1 to 2000)
			parts += "[span_redtext("ЦентКом недоволен. Причастные сотрудники получат штраф..")]<br>"
			award_service(/datum/award/achievement/jobs/service_bad)
		if(2001 to 4999)
			parts += "[span_greentext("ЦентКом удовлетворен работой сервиса на сегодняшний день.")]<br>"
			award_service(/datum/award/achievement/jobs/service_okay)
		else
			parts += "<span class='reallybig greentext'>ЦентКом невероятно впечатлен сегодняшним сервисом! Какая команда!</span><br>"
...
/datum/controller/subsystem/ticker/proc/hardcore_random_report()
	...
	for(var/mob/living/carbon/human/human_player in hardcores)
		. += "<li>[printplayer(human_player.mind)] с харжкорным случайным счетом [round(human_player.hardcore_survival_score)]</li>"
	. += "</ul></div>"
...
/datum/controller/subsystem/ticker/proc/give_show_report_button(client/C)
	...
	R.Grant(C.mob)
	to_chat(C,span_infoplain("<a href='byond://?src=[REF(R)];report=1'>Снова показать сводный отчет</a>"))
...
/datum/controller/subsystem/ticker/proc/cheevo_report()
	...
	if(length(GLOB.achievements_unlocked))
		...
		for(var/datum/achievement_report/cheevo_report in GLOB.achievements_unlocked)
			parts += "<BR>[cheevo_report.winner_key] был <b>[cheevo_report.winner]</b>, который получил достижение [span_greentext("'[cheevo_report.cheevo]'")] в [cheevo_report.award_location]!<BR>"
		parts += "</ul>"
