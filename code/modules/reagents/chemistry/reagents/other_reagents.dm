...
/datum/reagent/consumable/liquidgibs
	name = "Жидкий Гибс"
	color = "#CC4633"
	description = "Не стоит даже думать о том, что здесь находится."
	taste_description = "противный привкус железа"
	nutriment_factor = 2
...
/datum/reagent/bone_dust
	name = "Костная пыль"
	color = "#dbcdcb"
	description = "Перемолотые кости, мерзость!"
	taste_description = "самое отвратительное зерно на свете"
...
/datum/reagent/vaccine/fungal_tb
	name = "Вакцина (грибковый туберкулез)"
...
/datum/reagent/water
	name = "Вода"
	description = "Повсеместно распространенное химическое вещество, состоящее из водорода и кислорода."
...
/datum/reagent/water/salt
	name = "Солёная вода"
	description = "Вода, но соленая. Пахнет как... станционный лазарет?"
...
/datum/glass_style/drinking_glass/water/salt
	required_drink_type = /datum/reagent/water/salt
	name = "стакан солёной воды"
	desc = "Если у вас болит горло, прополощите его соленой водой, и боль уйдет. Можно использовать в качестве импровизированного местного средства против ран."
	icon_state = "glass_clear"
...
/datum/wound/pierce/bleed/on_saltwater(reac_volume, mob/living/carbon/carbies)
	...
	to_chat(carbies, span_notice("Соленая вода заливает [LOWER_TEXT(src)], впитывая кровь."))
...
/datum/wound/burn/flesh/on_saltwater(reac_volume)
	...
	to_chat(victim, span_notice("Соленая вода заливает [LOWER_TEXT(src)], впитывая... разные жидкости. После этого я чувствую себя немного лучше."))
	return

/datum/reagent/water/holywater
	name = "Святая вода"
	description = "Вода, освященная каким-то божеством."
...
/datum/glass_style/drinking_glass/holywater
	...
	name = "стакан святой воды"
	desc = "Стакан со святой водой."
	icon_state = "glass_clear"
...
/datum/reagent/water/holywater/on_mob_add(mob/living/affected_mob, amount)
	...
	if(IS_CULTIST(affected_mob))
		to_chat(affected_mob, span_userdanger("Мерзкая святость начинает протягивать свои сияющие щупальца через твой разум, очищая Геометра от влияния Крови!"))
...
/datum/reagent/water/holywater/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(IS_CULTIST(affected_mob))
		for(var/datum/action/innate/cult/blood_magic/BM in affected_mob.actions)
			...
			for(var/datum/action/innate/cult/blood_spell/BS in BM.spells)
				...
				qdel(BS)
			if(removed_any)
				to_chat(affected_mob, span_cult_large("Ваши кровавые обряды срываются, когда святая вода омывает ваше тело!"))
	...
	if(data["deciseconds_metabolized"] >= (25 SECONDS)) // 10 units
		...
		if(IS_CULTIST(affected_mob) && SPT_PROB(10, seconds_per_tick))
			...
			if(prob(10))
				affected_mob.visible_message(span_danger("[affected_mob] начинается припадок!"), span_userdanger("У тебя припадок!"))
				affected_mob.Unconscious(12 SECONDS)
				to_chat(affected_mob, span_cult_large("[pick("Ваша кровь - это ваши узы, без нее вы ничто", "Не забывайте о своем месте", \
					"Вся эта сила.. и вы все равно терпите неудачу?", "Если вы не можете избавиться от этого яда, я уничтожу вашу жалкую жизнь!")]."))
...
/datum/reagent/water/hollowwater
	name = "Полая вода"
	description = "Вездесущее химическое вещество, которое состоит из водорода и кислорода, но выглядит как бы полым."
	color = "#88878777"
...
/datum/reagent/hydrogen_peroxide
	...
	description = "Вездесущее химическое вещество, состоящее из водорода, кислорода и кислорода." //intended intended
	color = "#AAAAAA77" // rgb: 170, 170, 170, 77 (alpha)
	taste_description = "горячая вода"
	var/cooling_temperature = 2
	ph = 6.2
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/glass_style/drinking_glass/hydrogen_peroxide
	required_drink_type = /datum/reagent/hydrogen_peroxide
	name = "стакан оксигенированной воды"
	desc = "Отец всех прохладительных напитков. Вкус, конечно, отменный, верно?"
	icon_state = "glass_clear"
...
/datum/reagent/fuel/unholywater //if you somehow managed to extract this from someone, dont splash it on yourself and have a smoke
	name = "Несвятая вода"
	description = "То, что не должно существовать на этом свете."
...
/datum/reagent/hellwater //if someone has this in their system they've really pissed off an eldrich god
	name = "Адская вода"
	description = "ТВОЯ ПЛОТЬ! ОНА ГОРИТ!"
	taste_description = "горящая"
	ph = 0.1
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED|REAGENT_NO_RANDOM_RECIPE
...
/datum/reagent/lube
	...
	description = "Смазка - это вещество, вводимое между двумя движущимися поверхностями для уменьшения трения и износа между ними."
...
/datum/reagent/lube/superlube
	name = "Супер-пупер смазка"
	description = "Это \[ОТРЕДАКТИРОВАНО\] было объявлено вне закона после инцидента с \[ДАННЫЕ УДАЛЕНЫ\]."
	lube_kind = TURF_WET_SUPERLUBE
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED|REAGENT_NO_RANDOM_RECIPE

/datum/reagent/spraytan
	name = "Спрей-автозагар"
	description = "Вещество, наносимое на кожу для ее потемнения."
...
/datum/reagent/spraytan/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message = TRUE, touch_protection = 0)
	...
	if(ishuman(exposed_mob))
		...
		if((methods & INGEST) && show_message)
			to_chat(exposed_mob, span_notice("Вкус был ужасен."))
...
/datum/reagent/mutationtoxin
	name = "Стабильный мутационный токсин"
	description = "Гуманизирующий токсин."
	color = "#5EFF3B" //RGB: 94, 255, 59
	metabolization_rate = 0.5 * REAGENTS_METABOLISM //metabolizes to prevent micro-dosage
	taste_description = "слизь"
	var/race = /datum/species/human
	var/list/mutationtexts = list( "Ты не очень хорошо себя чувствуешь." = MUT_MSG_IMMEDIATE,
									"Ваша кожа выглядит немного ненормальной на ощупь." = MUT_MSG_IMMEDIATE,
									"Ваши конечности начинают приобретать другую форму." = MUT_MSG_EXTENDED,
									"Ваши придатки начинают трансформироваться." = MUT_MSG_EXTENDED,
									"Вам кажется, что вы можете измениться в любой момент!" = MUT_MSG_ABOUT2TURN)
...
/datum/reagent/mutationtoxin/on_mob_life(mob/living/carbon/human/affected_mob, seconds_per_tick, times_fired)
	...
	if(current_cycle >= CYCLES_TO_TURN)
		...
		to_chat(affected_mob, span_warning("Вы стали [LOWER_TEXT(initial(species_type.name))]!"))
		return

/datum/reagent/mutationtoxin/classic //The one from plasma on green slimes
	name = "Мутационный токсин"
	description = "Токсин, вызывающий разложение."
	color = "#13BC5E" // rgb: 19, 188, 94
	race = /datum/species/jelly/slime
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/mutationtoxin/felinid
	name = "Мутационный токсин кошачьих"
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/lizard
	name = "Мутационный токсин ящериц"
	description = "Токсин ящериц."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/fly
	name = "Мутационный токсин насекомых"
	description = "Токсин насекомых."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/moth
	name = "Мутационный токсин мотыльков"
...
/datum/reagent/mutationtoxin/pod
	name = "Мутационный токсин растений"
	description = "Растительный токсин."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/jelly
	name = "Токсин несовершенной мутации"
	description = "Желеобразный токсин."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/jelly/on_mob_life(mob/living/carbon/human/affected_mob, seconds_per_tick, times_fired)
	if(isjellyperson(affected_mob))
		to_chat(affected_mob, span_warning("Ваше желе изменяется и морфируется, превращая вас в другой подвид!"))
	...
	if(current_cycle >= CYCLES_TO_TURN) //overwrite since we want subtypes of jelly
		...
		affected_mob.set_species(species_type)
		holder.del_reagent(type)
		to_chat(affected_mob, span_warning("Вы стали [initial(species_type.name)]!"))
		return UPDATE_MOB_HEALTH
	return ..()

/datum/reagent/mutationtoxin/golem
	name = "Мутационный токсин големов"
	description = "Кристаллический токсин."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/abductor
	name = "Мутационный токсин абдуктора"
	description = "Инопланетный токсин."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/android
	name = "Мутационный токсин андроидов"
	description = "Токсин роботов."
	color = "#5EFF3B" //RGB: 94, 255, 59
	race = /datum/species/android
	taste_description = "электросхемы и сталь"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED|REAGENT_NO_RANDOM_RECIPE

//BLACKLISTED RACES
/datum/reagent/mutationtoxin/skeleton
	name = "Мутационный токсин скелета"
	description = "Страшный токсин."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/zombie
	name = "Мутационный токсин зомби"
	description = "Токсин нежити."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/ash
	name = "Мутационный токсин пепла"
	description = "Пепельный токсин."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/shadow
	name = "Мутационный токсин тени"
	description = "Мрачный токсин."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mutationtoxin/plasma
	name = "Мутационный токсин плазмы"
	description = "Токсин на основе плазмы."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mulligan
	name = "Токсин Маллигана"
	description = "Этот токсин быстро изменяет ДНК гуманоидных существ. Обычно используется шпионами и убийцами Синдиката, нуждающимися в срочной смене идентификатора."
	color = "#5EFF3B" //RGB: 94, 255, 59
...
/datum/reagent/mulligan/on_mob_life(mob/living/carbon/human/affected_mob, seconds_per_tick, times_fired)
	...
	if (!istype(affected_mob))
		return
	to_chat(affected_mob, span_warning("<b>Вы скрипите зубами от боли, пока ваше тело стремительно мутирует!</b>"))
	affected_mob.visible_message("<b>[affected_mob]</b> внезапно трансформируется!")
	randomize_human_normie(affected_mob)

/datum/reagent/aslimetoxin
	name = "Продвинутый мутационный токсин"
	description = "Усовершенствованный токсин, вызывающий разложение, вырабатываемый слизью."
	color = "#13BC5E" // rgb: 19, 188, 94
	taste_description = "слизь"
	penetrates_skin = NONE
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/gluttonytoxin
	name = "Благословение обжоры"
	description = "Продвинутый токсин, вызывающий разложение, вырабатываемый чем-то ужасным."
	color = "#5EFF3B" //RGB: 94, 255, 59
	taste_description = "разложение"
	penetrates_skin = NONE
...
/datum/reagent/serotrotium
	name = "Серотротиум"
	description = "Химическое соединение, способствующее концентрированной выработке нейротрансмиттера серотонина в организме человека."
...
/datum/reagent/oxygen
	name = "Кислород"
	description = "Газ без цвета и запаха. Растет на деревьях, но все равно является довольно ценным."
	color = COLOR_GRAY
...
/datum/reagent/copper
	name = "Медь"
	description = "Это очень пластичный металл. Изделия, изготовленные из меди, не очень долговечны, но из них получается неплохой материал для электропроводки."
...
/datum/reagent/nitrogen
	name = "Азот"
	description = "Газ без цвета, запаха и вкуса. Простое удушающее средство, которое может бесшумно вытеснять жизненно необходимый кислород."
	color = COLOR_GRAY
	taste_mult = 0
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/hydrogen
	name = "Водород"
	description = "Двухатомный газ без цвета, запаха, неметаллический, безвкусный, легко воспламеняющийся."
	color = COLOR_GRAY
	taste_mult = 0
...
/datum/reagent/potassium
	name = "Калий"
	description = "Мягкое, легкоплавкое вещество, которое легко режется ножом. Активно реагирует с водой."
	color = "#A0A0A0" // rgb: 160, 160, 160
	taste_description = "сладкий"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/mercury
	name = "Ртуть"
	description = "Необычный металл, который остается жидким при комнатной температуре. Вызывает нейродегенерацию и очень вреден для психики."
...
/datum/reagent/sulfur
	name = "Сера"
	description = "Болезненно-желтое вещество, известное в основном своим неприятным запахом. На самом деле оно гораздо полезнее, чем кажется с точки зрения биохимии."
...
/datum/reagent/carbon
	name = "Углерод"
	description = "Рассыпчатое черное вещество, которое, хотя и не вызывает восторга на физическом уровне, составляет основу всей известной жизни. Это что-то особенное."
...
/datum/reagent/chlorine
	name = "Хлор"
	description = "Бледно-желтый газ, хорошо известный как окислитель. Хотя в своей элементарной форме он образует много безвредных молекул, на самом деле он далеко не безвреден."
	color = "#FFFB89" //pale yellow? let's make it light gray
	taste_description = "хлор"
	ph = 7.4
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/fluorine
	name = "Фтор"
	description = "Комично-реактивный химический элемент. Вселенная ни в малейшей степени не желает, чтобы это вещество существовало в таком виде."
	color = COLOR_GRAY
...
/datum/reagent/sodium
	name = "Натрий"
	description = "Мягкий серебристый металл, который легко режется ножом. Это еще не соль, поэтому не добавляйте ее в чипсы."
	color = COLOR_GRAY
	taste_description = "соленый металл"
	ph = 11.6
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/phosphorus
	name = "Фосфор"
	description = "Румяный порошок, который легко сгорает. Хотя он бывает разных цветов, общая тема всегда одна и та же."
...
/datum/reagent/lithium
	name = "Литий"
	description = "Это серебристый металл, который славится своей удивительно низкой плотностью. Его использование слишком эффективно для самоуспокоения."
	color = COLOR_GRAY
	taste_description = "металл"
	ph = 11.3
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/space_cleaner/sterilizine
	name = "Стерилизин"
	description = "Стерилизует раны при подготовке к операции."
...
/datum/reagent/silver
	...
	taste_description = "дорогой и приемлемый металл"
...
/datum/reagent/uranium
	...
	taste_description = "внутренняя часть реактора"
	ph = 4
...
/datum/reagent/uranium/radium
	...
	color = "#00CC00" // ditto
	taste_description = "голубой цвет и сожаление"
	tox_damage = 1
	material = null
	ph = 10
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/bluespace
	name = "Блюспейс пыль"
	description = "Пыль, состоящая из микроскопических кристаллов блюспейса, обладающих незначительными свойствами искривления пространства."
	color = "#0000CC"
	taste_description = "шипучая синева"
...
/datum/reagent/bluespace/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(current_cycle > 10 && SPT_PROB(7.5, seconds_per_tick))
		to_chat(affected_mob, span_warning("Вы чувствуете себя неуравновешенным..."))
...
/datum/reagent/aluminium
	name = "Алюминий"
	description = "Серебристо-белый и пластичный элемент из группы химических элементов бор."
...
/datum/reagent/silicon
	name = "Кремний"
	description = "Кремний, четырехвалентный металлоид, менее реакционноспособен, чем его химический аналог углерод."
	color = "#A8A8A8" // rgb: 168, 168, 168
	taste_mult = 0
...
/datum/reagent/fuel
	...
	taste_description = "противный металл"
	penetrates_skin = NONE
	ph = 4
...
/datum/glass_style/drinking_glass/fuel
	...
	name = "стакан с топливом"
	desc = "Если вы не пользуетесь промышленным инструментом, это, вероятно, небезопасно для употребления."
...
/datum/reagent/space_cleaner
	name = "Космочист"
	description = "Состав, используемый для чистки предметов. Теперь гипохлорита натрия стало на 50% больше! Можно использовать для очистки ран, но на самом деле он не предназначен для этого."
...
/datum/reagent/space_cleaner/on_burn_wound_processing(datum/wound/burn/flesh/burn_wound)
	...
	if(prob(5))
		to_chat(burn_wound.victim, span_notice("Твоя [burn_wound] щиплет и обжигает от того, что [src] прикрывает ее! Хотя это <i>действительно</i> выглядит довольно чисто."))
...
/datum/reagent/space_cleaner/ez_clean
	name = "EZ Чистый"
	description = "Мощное кислотное чистящее средство, продаваемое компанией Waffle Corp. Воздействует на органические вещества, не затрагивая другие предметы."
...
/datum/reagent/impedrezene
	name = "Импедрезин"
	description = "Импедрезен - это наркотик, который замедляет высшие функции клеток мозга. "
...
/datum/reagent/cyborg_mutation_nanomachines
	name = "Наномашины"
	description = "Микроскопические строительные роботы. Это Наномашины, сынок!"
...
/datum/reagent/xenomicrobes
	name = "Ксеномикробы"
	description = "Микробы с совершенно чуждой клеточной структурой."
	color = "#535E66" // rgb: 83, 94, 102
	taste_description = "осадок"
	penetrates_skin = NONE
...
/datum/reagent/snail
	name = "Агент-S"
	description = "Вирусологический агент, который заражает субъекта гастролезом."
	color = COLOR_VERY_DARK_LIME_GREEN // rgb(0, 51, 0)
	taste_description = "липкое"
	penetrates_skin = NONE
	ph = 11
...
/datum/reagent/foaming_agent// Metal foaming agent. This is lithium hydride. Add other recipes (e.g. LiH + H2O -> LiOH + H2) eventually.
	...
	description = "Средство, которое образует металлическую пену при смешивании с легким металлом и сильной кислотой."
...
/datum/reagent/smart_foaming_agent //Smart foaming agent. Functions similarly to metal foam, but conforms to walls.
	name = "Умный пенообразующий агент"
	description = "Средство, которое при смешивании с легким металлом и сильной кислотой образует металлическую пену, соответствующую границам поверхности."
	color = "#664B63" // rgb: 102, 75, 99
	taste_description = "металл"
	ph = 11.8
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/diethylamine
	name = "Диэтиламин"
	description = "Вторичный амин, слабо коррозийный."
...
/datum/reagent/carbondioxide
	...
	description = "Газ, который обычно образуется при сжигании углеродного топлива. Вы постоянно выделяете его в своих легких."
...
/datum/reagent/nitrous_oxide
	name = "Оксид азота"
	description = "Сильнодействующий окислитель, используемый в качестве топлива в ракетах и анестетика во время операций. Поскольку это антикоагулянт, закись азота лучше всего \
		использовать вместе с сангвиритом, чтобы свертываемость крови продолжалась."
	metabolization_rate = 1.5 * REAGENTS_METABOLISM
	color = COLOR_GRAY
	taste_description = "сладкий"
	ph = 5.8
...
/datum/reagent/colorful_reagent/powder
	name = "Обычный порошок" //the name's a bit similar to the name of colorful reagent, but hey, they're practically the same chem anyway
	description = "Порошок, который используется для окрашивания вещей."
	color = COLOR_WHITE
	taste_description = "задние ряды класса"
	can_color_organs = TRUE
	var/colorname = "none"

/datum/reagent/colorful_reagent/powder/New()
	if(colorname == "none")
		description = "Довольно обыденный на вид порошок. Не похоже, что он может что-то сильно подкрасить..."
	else if(colorname == "invisible")
		description = "Невидимый порошок. К сожалению, поскольку он невидимый, не похоже, что он может что-либо подкрасить..."
	else
		description = "[colorname] порошок, используемый для окрашивания вещей в [colorname]."
	return ..()
...
/datum/reagent/colorful_reagent/powder/black/crayon
	name = "Чёрный карандашный порошок"
	can_color_mobs = FALSE
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/colorful_reagent/powder/white/crayon
	name = "Белый карандашный порошок"
	can_color_mobs = FALSE
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/plantnutriment
	...
	description = "Какое-то питательное вещество. Вы не можете точно сказать, что это такое. Вероятно, вам следует сообщить об этом, а также о том, как вы его получили."
...
/datum/reagent/plantnutriment/eznutriment
	name = "Питательное вещество E-Z"
	description = "Содержит электролиты. Это то, чего так жаждут растения."
...
/datum/reagent/plantnutriment/left4zednutriment
	name = "Лев-4-Зед"
	description = "Нестабильное питание, заставляющее растения мутировать чаще, чем обычно."
...
/datum/reagent/plantnutriment/robustharvestnutriment
	...
	description = "Очень мощное питательное вещество, замедляющее мутацию растений."
...
/datum/reagent/plantnutriment/endurogrow
	name = "Эндуро-рост"
	description = "Специализированное питательное средство, которое уменьшает количество продукта и его эффективность, но повышает выносливость растений."
...
/datum/reagent/plantnutriment/liquidearthquake
	name = "Жидкое землетрясение"
	description = "Специализированное питательное средство, которое увеличивает скорость роста растений, а также их восприимчивость к сорнякам."
...
/datum/reagent/fuel/oil
	name = "Масло"
	description = "Сжигается на небольшом дымном костре, может быть использован для получения пепла."
	color = "#2D2D2D"
...
/datum/reagent/stable_plasma
	...
	description = "Негорючая плазма, заключенная в жидкую форму, которая не может воспламениться или стать газообразной/твердой."
	color = "#2D2D2D"
	taste_description = "горький"
	taste_mult = 1.5
	ph = 1.5
...
/datum/reagent/iodine
	name = "Йод"
	description = "Обычно добавляется в поваренную соль в качестве питательного вещества. Сама по себе она гораздо менее приятна на вкус."
	color = "#BC8A00"
	taste_description = "металл"
	ph = 4.5
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/carpet
	name = "Ковёр"
	description = "Для тех, кому нужен более креативный способ расстелить красную дорожку."
	color = "#771100"
...
/datum/reagent/carpet/black
	name = "Чёрный ковёр"
	description = "ковёр тоже входит в комплект... БЛАПК" //yes, the typo is intentional
	color = "#1E1E1E"
...
/datum/reagent/carpet/blue
	name = "Синий ковёр"
	description = "Для тех, кому действительно нужно немного расслабиться."
	color = "#0000DC"
	taste_description = "замерзший ковёр"
	carpet_type = /turf/open/floor/carpet/blue
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/carpet/cyan
	name = "Головой ковёр"
	description = "Для тех, кому нужно вспомнить годы использования яда в качестве строительного материала. Пахнет асбестом."
	color = "#00B4FF"
...
/datum/reagent/carpet/green
	name = "Зеленый ковёр"
	description = "Для тех, кому нужна идеальная начинка для зеленых яиц и ветчины."
	color = COLOR_CRAYON_GREEN
	taste_description = "Зеленый" //the caps is intentional
	carpet_type = /turf/open/floor/carpet/green
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/carpet/orange
	name = "Оранжевый ковёр"
	description = "Для тех, кто предпочитает здоровый ковёр в дополнение к своему здоровому питанию."
	color = "#E78108"
...
/datum/reagent/carpet/purple
	...
	description = "Для тех, кому нужно растратить большое количество целебного желе, чтобы выглядеть модно."
	color = "#91D865"
...
/datum/reagent/carpet/red
	name = "Красный ковёр"
	description = "Для тех, кому нужна ковровая дорожка еще более красного цвета."
	color = "#731008"
	taste_description = "кровь и потроха"
	carpet_type = /turf/open/floor/carpet/red
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/carpet/royal
	name = "Королевский ковёр?"
	description = "Для тех, кто ломает игру и нуждается в сообщении о проблеме."
...
/datum/reagent/carpet/royal/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(liver)
		...
		if(HAS_TRAIT(liver, TRAIT_ROYAL_METABOLISM))
			if(SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, "Ты чувствуешь себя членом королевской семьи.")
			if(SPT_PROB(2.5, seconds_per_tick))
				affected_mob.say(pick("Крестьяне..","Этот ковёр стоит больше, чем ваши контракты!","Я могу уволить тебя в любой момент..."), forced = "royal carpet")
		...
		// The quartermaster, as a semi-head, has a "pretender royal" metabolism
			...
			if(SPT_PROB(8, seconds_per_tick))
				to_chat(affected_mob, "Ты чувствуешь себя самозванцем...")

/datum/reagent/carpet/royal/black
	name = "Королевский черный ковёр"
	description = "Для тех, кто чувствует потребность продемонстрировать свои навыки, позволяющие не тратить время впустую."
	color = COLOR_BLACK
...
/datum/reagent/carpet/royal/blue
	name = "Королевский синий ковёр"
	description = "Для тех, кто чувствует потребность продемонстрировать свои навыки, позволяющие не тратить время впустую... в СИНЕМ."
	color = "#5A64C8"
	taste_description = "блюялти" //also intentional
...
/datum/reagent/carpet/neon/simple_white
	...
	description = "Для тех, кто любит люминесцентное освещение."
	color = LIGHT_COLOR_HALOGEN
	taste_description = "пары натрия"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/carpet/neon/simple_red
	...
	description = "Для тех, кто любит немного неопределенности."
	color = COLOR_RED
...
/datum/reagent/carpet/neon/simple_yellow
	name = "Простой желтый неоновый ковёр"
	description = "Для тех, кому нужно немного стабильности в своей жизни."
	color = COLOR_YELLOW
	taste_description = "стабильный неон"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
	carpet_type = /turf/open/floor/carpet/neon/simple/yellow

/datum/reagent/carpet/neon/simple_lime
	name = "Простой лаймово-неоновый ковёр"
	description = "Для тех, кому нужно немного горечи."
	color = COLOR_LIME
	taste_description = "неоновый цитрусовый"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
	carpet_type = /turf/open/floor/carpet/neon/simple/lime

/datum/reagent/carpet/neon/simple_green
	name = "Простой зеленый неоновый ковёр"
	description = "Для тех, кому нужно немного изменить свою жизнь."
	color = COLOR_GREEN
...
/datum/reagent/carpet/neon/simple_teal
	...
	description = "Для тех, кому нужно покурить."
	color = COLOR_TEAL
...
/datum/reagent/carpet/neon/simple_blue
	name = "Простой синий неоновый ковер"
	description = "Для тех, кому нужно снова почувствовать радость."
	color = COLOR_NAVY
	taste_description = "неоново-синий"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
	carpet_type = /turf/open/floor/carpet/neon/simple/blue

/datum/reagent/carpet/neon/simple_purple
	name = "Простой пурпурный неоновый ковер"
	description = "Для тех, кто нуждается в небольшом исследовании."
	color = COLOR_PURPLE
...
/datum/reagent/carpet/neon/simple_violet
	name = "Простой фиолетовый неоновый ковер"
...
/datum/reagent/pentaerythritol
	name = "Пентаэритрит"
	description = "Притормози, это тебе не конкурс по правописанию!"
	color = "#E66FFF"
	taste_description = "кислота"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/acetaldehyde
	name = "Ацетальдегид"
	description = "Похоже на пластик. На вкус как мертвец."
	color = "#EEEEEF"
	taste_description = "мертвечина" //made from formaldehyde, ya get da joke ?
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/acetone_oxide
	name = "Оксид ацетона"
	description = "Порабощенный кислород"
	color = "#966199cb"
	taste_description = "кислота"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/phenol
	name = "Фенол"
	description = "Ароматическое углеродное кольцо с гидроксильной группой. Полезный предшественник некоторых лекарств, но сам по себе не обладает целебными свойствами."
	color = "#E7EA91"
	taste_description = "кислота"
	ph = 5.5
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/ash
	name = "Пепел"
	description = "Предположительно, из этого воскрешаются фениксы, но вы никогда этого не видели."
	color = "#515151"
...
/datum/reagent/acetone
	name = "Ацетон"
	description = "Вязкая, слегка канцерогенная жидкость. Имеет множество повседневных применений в повседневной жизни."
	color = "#AF14B7"
	taste_description = "кислота"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/colorful_reagent
	name = "Цветастый реагент"
	description = "Тщательно пробуйте радугу на вкус."
...
/datum/reagent/hair_dye
	...
	description = "Есть большая вероятность, что вы будете выглядеть как сумасшедший ученый."
...
/datum/reagent/barbers_aid
	name = "Помощь парикмахера"
	description = "Решение проблемы выпадения волос во всем мире."
	color = "#A86B45" //hair is brown
	taste_description = "стильно"
	penetrates_skin = NONE
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/barbers_aid/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message=TRUE, touch_protection = 0)
	...
	to_chat(exposed_human, span_notice("Волосы начинают прорастать на [HAS_TRAIT(exposed_human, TRAIT_BALD) ? "лице" : "коже головы"]."))

/datum/reagent/concentrated_barbers_aid
	name = "Концентрированная помощь парикмахеру"
	description = "Концентрированное средство от выпадения волос по всему миру."
...
/datum/reagent/concentrated_barbers_aid/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message=TRUE, touch_protection = 0)
	...
	to_chat(exposed_human, span_notice("Ваши волосы [HAS_TRAIT(exposed_human, TRAIT_BALD) ? " лица" : ""] начинают расти с невероятной скоростью!"))
...
/datum/reagent/concentrated_barbers_aid/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(current_cycle > 21 / creation_purity)
		...
		if(HAS_TRAIT(affected_mob, TRAIT_BALD))
			to_chat(affected_mob, span_warning("Вы чувствуете, как меняется ваша кожа головы, но вы по-прежнему безнадежно лысы."))
		else
			to_chat(affected_mob, span_notice("Ваша кожа головы мутирует, и из нее вырастает целая шевелюра."))
			human_mob.update_body_parts()

/datum/reagent/baldium
	name = "Лысиум"
	description = "Это основная причина выпадения волос во всем мире."
	color = "#ecb2cf"
	taste_description = "кризис среднего возраста"
	penetrates_skin = NONE
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/baldium/expose_mob(mob/living/exposed_mob, methods=TOUCH, reac_volume, show_message=TRUE, touch_protection = 0)
	...
	var/mob/living/carbon/human/exposed_human = exposed_mob
	to_chat(exposed_human, span_danger("У тебя волосы выпадают клочьями!"))
	exposed_human.set_facial_hairstyle("Побритый", update = FALSE)
	exposed_human.set_hairstyle("Лысый", update = TRUE)

/datum/reagent/saltpetre
	name = "Селитра"
	description = "Советская дымовуха"
	color = "#60A584" // rgb: 96, 165, 132
	taste_description = "холодная соль"
	ph = 11.2
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/lye
	name = "Щелочь"
	description = "Также известен как гидроксид натрия. Профессия, связанная с этим, несколько обескураживает."
...
/datum/reagent/drying_agent
	name = "Сушащий агент"
	description = "Осушитель. Можно использовать для сушки вещей."
	color = "#A70FFF"
...
/datum/reagent/uranium/uraniumvirusfood
	name = "Гель распадающегося урана"
...
/datum/reagent/royal_bee_jelly
	name = "Королевское пчелиное молоко"
	description = "Маточное пчелиное молочко, если ввести его в пчелиную матку, разделится на две части."
	color = "#00ff80"
...
/datum/reagent/romerol
	...
	description = "Ромерол - это экспериментальный биотеррористический препарат \
		который вызывает дремлющие узлы, вытравливаемые в сером веществе \
		субъекта. Эти узлы становятся активными только после смерти хозяина, \
		после чего вторичные структуры активизируются и берут под контроль \
		тело хозяина."
...
/datum/reagent/magillitis
	name = "Магиллитис"
	description = "Экспериментальная сыворотка, которая вызывает быстрый рост мышечной массы у гоминидов. Побочные эффекты могут включать гипертрихоз, вспышки ярости и непреодолимую тягу к бананам."
	color = "#00f041"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED|REAGENT_NO_RANDOM_RECIPE
...
/datum/reagent/growthserum
	...
	description = "Коммерческое химическое средство, предназначенное для помощи пожилым мужчинам в постели."//not really it just makes you a giant
	color = "#ff0000"//strong red. rgb 255, 0, 0
	var/current_size = RESIZE_DEFAULT_SIZE
	taste_description = "горький" // apparently what viagra tastes like
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/plastic_polymers
	...
	description = "пластмассовые компоненты на нефтяной основе."
	color = "#f7eded"
...
/datum/reagent/glitter
	name = "Универсальный блеск"
	description = "если вы видите это описание, обратитесь к кодеру."
...
/datum/reagent/glitter/pink
	name = "Розовый блеск"
	description = "розовые блестки, которые проникают повсюду"
...
/datum/reagent/glitter/white
	name = "Белый блеск"
	description = "белые блестки, которые проникают повсюду"
	glitter_type = /obj/effect/decal/cleanable/glitter/white
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/glitter/blue
	name = "Синий блеск"
	description = "синие блестки, которые проникают повсюду"
...
/datum/reagent/glitter/confetti
	name = "Конфетти"
	description = "Крошечные пластиковые хлопья, которые невозможно смахнуть."
	color = "#7dd87b"
...
/datum/reagent/pax
	name = "Пакс"
	description = "Бесцветная жидкость, которая подавляет насилие в своих субъектах.."
	color = "#AAAAAA55"
	taste_description = "вода"
	metabolization_rate = 0.25 * REAGENTS_METABOLISM
	ph = 15
...
/datum/reagent/bz_metabolites
	name = "Метаболиты БЗ"
	description = "Безвредный метаболит газа BZ."
	color = "#FAFF00"
	taste_description = "острая корица"
...
/datum/reagent/pax/peaceborg
	name = "Синтпакс"
	description = "То же самое, что и Пакс, но метаболизируется гораздо быстрее. Может производить только Борг-миротворец.	"
	metabolization_rate = 1.5 * REAGENTS_METABOLISM
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED|REAGENT_NO_RANDOM_RECIPE

/datum/reagent/peaceborg/confuse
	name = "Головокружительный раствор"
	description = "Цель теряет равновесие и у нее кружится голова"
	metabolization_rate = 1.5 * REAGENTS_METABOLISM
	taste_description = "головокружительно"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED|REAGENT_NO_RANDOM_RECIPE
...
/datum/reagent/peaceborg/confuse/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(SPT_PROB(10, seconds_per_tick))
		to_chat(affected_mob, "Вы чувствуете себя сбитым с толку и дезориентированным.")

/datum/reagent/peaceborg/tire
	name = "Изнурительное решение"
	description = "Чрезвычайно слабый токсин, повышающий выносливость, который утомляет цель. Абсолютно безвреден."
	metabolization_rate = 1.5 * REAGENTS_METABOLISM
...
/datum/reagent/gondola_mutation_toxin
	name = "Транквилити"
	description = "Жидкость с высокой степенью мутации неизвестного происхождения."
...
/datum/reagent/spider_extract
	name = "Экстракт паука"
	description = "Высокоспециализированный экстракт, добываемый в секторе Australicus, используемый для создания пауков-праматерей."
	color = "#ED2939"
	taste_description = "вверх ногами"

/// Improvised reagent that induces vomiting. Created by dipping a dead mouse in welder fluid.
/datum/reagent/yuck
	name = "Органическая суспензия"
	description = "Смесь жидкости разного цвета. Вызывает рвоту."
	color = "#545000"
	taste_description = "нутро"
	taste_mult = 4
...
/datum/glass_style/drinking_glass/yuck
	...
	name = "стакан... фу!"
	desc = "Пахнет от него как от туши, да и выглядит ненамного лучше."
...
/datum/reagent/yuck/on_mob_life(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	...
	if(!yuck_cycle)
		if(SPT_PROB(4, seconds_per_tick))
			var/dread = pick("Что-то шевелится у тебя в животе...", \
				"Из твоего желудка раздается урчание...", \
				"На мгновение тебе кажется, что все вокруг движется, но это твой желудок...")
...
/datum/reagent/monkey_powder
	name = "Обезьяний порошок	"
	description = "Просто добавьте воды!"
	color = "#9C5A19"
	taste_description = "бананы"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED

/datum/reagent/plasma_oxide
	name = "Гиперплазменный оксид"
	description = "Вещество, образующееся глубоко в ядрах планет класса демонов. Обычно встречается в глубинных гейзерах."
...
/datum/reagent/exotic_stabilizer
	...
	description = "Усовершенствованный состав, созданный путем смешивания стабилизатора и гиперплазмиевого оксида."
...
/datum/reagent/wittel
	name = "Виттел"
	description = "Чрезвычайно редкое вещество металлически-белого цвета, встречающееся только на планетах класса демонов."
...
/datum/reagent/metalgen
	...
	description = "Фиолетовая жидкость, напоминающая металл, которая, как говорят, придает свои металлические свойства всему, к чему прикасается."
	color = "#b000aa"
	taste_mult = 0 // oderless and tasteless
...
/datum/reagent/gravitum
	name = "Гравитиум"
	description = "Редкий вид нулевой жидкости, способный временно сбрасывать вес со всего, к чему прикасается." //i dont even
...
/datum/reagent/cellulose
	...
	description = "Кристаллический полимер полидекстрозы - растения обожают это вещество."
	color = "#E6E6DA"
	taste_mult = 0
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/determination
	name = "Решительность"
	description = "Для тех случаев, когда вам нужно поднажать еще немного. НЕ допускайте приближения к растениям."
	color = "#D2FFFA"
...
/datum/reagent/eldritch
	name = "Эльдритская эссенция"
	description = "Странная жидкость, которая не подчиняется законам физики. \
		Она заряжает энергией и исцеляет тех, кто способен видеть за пределами этой хрупкой реальности, \
		но невероятно вредна для закрытых людей. Очень быстро метаболизируется."
...
/datum/reagent/universal_indicator
	...
	description = "Раствор, который можно использовать для создания индикаторов из pH-бумаги или распылять на предметы, чтобы придать им цвет в соответствии с их pH."
...
/datum/reagent/ants
	name = "Муравьи"
	description = "Это генетическая помесь муравьев и термитов, их укусы причиняют боль в 3 балла по шкале Шмидта."
	color = "#993333"
	taste_mult = 1.3
	taste_description = "крошечные ножки пробираются по задней стенке твоего горла"
...
/datum/reagent/ants/on_mob_life(mob/living/carbon/victim, seconds_per_tick)
	...
	if(SPT_PROB(5, seconds_per_tick))
		if(SPT_PROB(5, seconds_per_tick)) //Super rare statement
			victim.say("О, НЕТ, ТОЛЬКО НЕ МУРАВЬИ! ТОЛЬКО НЕ МУРАВЬИ! УАГХ, ОНИ У МЕНЯ В ГЛАЗАХ! МОИ ГЛАЗА! УАГХ!!", forced = type)
		else
...
/datum/reagent/ants/on_mob_end_metabolize(mob/living/living_anthill)
	. = ..()
	ant_ticks = 0
	to_chat(living_anthill, span_notice("Вы чувствуете, что последний из [src] вышел из вашей системы."))
...
/datum/reagent/ants/fire
	...
	description = "Редкая мутация космических муравьев, появившаяся в результате воздействия плазменного огня. Их укусы вызывают боль в 3,7 балла по шкале Шмидта."
	color = "#b51f1f"
	taste_description = "крошечные пылающие ножки пробираются по задней стенке твоего горла"
...
/datum/glass_style/drinking_glass/fire_ants
	...
	name = "стакан с огненными муравьями"
...
/datum/reagent/lead
	name = "Свинец"
	description = "Матовый металлический элемент с низкой температурой плавления."
	taste_description = "металл"
	color = "#80919d"
	metabolization_rate = 0.4 * REAGENTS_METABOLISM
...
/datum/reagent/kronkus_extract
	name = "Экстракт Кронкуса"
	description = "Пенистый экстракт, приготовленный из ферментированной мякоти винограда сорта кронкус.Обладает высокой горечью из-за присутствия различных кронкаминов."
	taste_description = "горький"
	color = "#228f63"
...
/datum/reagent/brimdust
	name = "Пыль с краев"
	description = "Демоническая пыльца. Не рекомендуется употреблять в пищу, хотя растениям она нравится."
	color = "#522546"
	taste_description = "горящий"
	chemical_flags = REAGENT_CAN_BE_SYNTHESIZED
...
/datum/reagent/love
	name = "Любовь"
	description = "Эта еда приготовлена... с любовью."
	color = "#ff7edd"
...
/datum/reagent/hauntium
	name = "Хаунтиум"
	color = "#3B3B3BA3"
	description = "Жуткая жидкость, созданная для очищения от присутствия призраков. Если она попадет в ваше тело, то начнет причинять боль вашей душе." //soul as in mood and heart
...
/datum/reagent/hauntium/on_mob_metabolize(mob/living/carbon/affected_mob, seconds_per_tick, times_fired)
	. = ..()
	to_chat(affected_mob, span_userdanger("Вы чувствуете присутствие зла внутри себя!"))
...
/datum/reagent/luminescent_fluid
	name = "Зеленая светящаяся жидкость"
	description = "Окрашенная жидкость, которая излучает свет в результате химической реакции с кислородом." // Reacts with oxygen in hydrogen peroxide IRL
...
/datum/reagent/luminescent_fluid/red
	name = "Красная светящаяся жидкость"
